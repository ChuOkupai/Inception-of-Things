...
kubectl wait --for=condition=available --timeout=300s deployment/will-playground -n dev
echo "Application is ready!"

# 6. Apply your application
kubectl apply -f confs/app.yaml

# 7. Now do port-forwarding
...
kubectl port-forward svc/will-playground-service -n dev 8888:8888 &
echo "Application port forwarding started!"




# 8. Get the ArgoCD admin password
echo "ArgoCD admin password:"
kubectl -n argocd get secret argocd-initial-admin-secret -o jsonpath="{.data.password}" | base64 -d
echo
echo "ArgoCD UI is available at http://localhost:8080"

# 9. Check if the application is available
